...
type DomainEvent = ExpenseSubmitted | ExpenseMovedToReview | ExpenseApproved | ExpenseRejected;
...
export class ExpenseRequest {
  ...
  private constructor(readonly id: ExpenseRequestId, readonly amount: Money) {
  ...
  private assertNotDecided(attempted: string): void {
    if (this.state === ExpenseState.Approved || this.state === ExpenseState.Rejected) {
      throw new InvalidStateTransitionError(this.state, attempted);
    };
  };

  moveToReview(): void {
    if (this.state !== ExpenseState.Pending) throw new InvalidStateTransitionError(this.state, "moveToReview");
  ...
  recordApproval(level: ApprovalLevel, approverId: string, threshold: Money): void {
    this.assertNotDecided("recordApproval");
    if (!this.debitAccountId || !this.creditAccountId) throw new MissingDestinationAccountsError();
    ...
    if (requiresBothLevels && level === ApprovalLevel.Finance && !this.hasApprovalFrom(ApprovalLevel.Manager)) {
      throw new OutOfOrderApprovalError(ApprovalLevel.Finance, ApprovalLevel.Manager);
    };

    this.approvals.push(Approval.of(level, approverId));

    const isFullyApproved =
      !requiresBothLevels || (this.hasApprovalFrom(ApprovalLevel.Manager) && this.hasApprovalFrom(ApprovalLevel.Finance));
