import type { ApprovalLevel, ExpenseState } from "./expense.enums";
...
export class MissingDestinationAccountsError extends Error {
	constructor(cause?: unknown) {
		super(
			`no se puede aprobar sin cuentas contables (debit/credit) asignadas`,
			{ cause },
		);
...
export class OutOfOrderApprovalError extends Error {
	constructor(
		readonly attempted: ApprovalLevel,
		readonly required: ApprovalLevel,
		cause?: unknown,
	) {
		super(
			`no se puede aprobar como "${attempted}" antes de que "${required}" apruebe`,
			{ cause },
		);
